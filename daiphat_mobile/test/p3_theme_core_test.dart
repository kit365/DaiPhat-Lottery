...
void main() {
  ...
  group('P3.1 - AppColors Backward Compatibility Aliases', () {
    test('all 24 aliases retain expected ARGB values and match semantic tokens', () {
      // 1. primary
      ...
      expect(AppColors.loginPlaceholder, equals(AppColors.contentPlaceholder));
  ...
  group('P3.1 - AppTheme Configuration & Component Wiring', () {
    test('bundled typography maps semantic roles to their families', () {
      ...
      expect(AppTypography.number().fontFamily, equals(AppTypography.displayFamily));
      expect(
    ...
    test('general ThemeData properties and ColorScheme', () {
      ...
      expect(sharedTheme.scaffoldBackgroundColor, equals(AppColors.backgroundPrimary));
      ...
      expect(sharedTheme.colorScheme.onPrimary, equals(AppColors.surfacePrimary));
      expect(sharedTheme.colorScheme.onSecondary, equals(AppColors.contentPrimary));
      expect(sharedTheme.colorScheme.onSurface, equals(AppColors.contentPrimary));
    });
    ...
    test('AppBarTheme properties', () {
      ...
      expect(appBarTheme.surfaceTintColor, equals(AppColors.surfaceTintTransparent));
    ...
    test('InputDecorationTheme properties', () {
      ...
      expect(inputTheme.hintStyle?.fontFamily, equals(AppTypography.mainFamily));
    });
    ...
    testWidgets('pumps light MaterialApp and verifies resolved theme preservation', (tester) async {
      await tester.pumpWidget(
        MaterialApp(
          theme: sharedTheme,
          home: Scaffold(
            appBar: AppBar(
              title: const Text('Theme Test Title'),
            ),
            body: Builder(
      ...
      expect(resolvedTheme.scaffoldBackgroundColor, equals(AppColors.backgroundPrimary));
      expect(resolvedTheme.colorScheme.primary, equals(AppColors.brandPrimary));
      expect(resolvedTheme.colorScheme.secondary, equals(AppColors.brandAccent));
      expect(resolvedTheme.colorScheme.surface, equals(AppColors.surfacePrimary));
      expect(resolvedTheme.colorScheme.error, equals(AppColors.statusError));
      expect(resolvedTheme.colorScheme.onPrimary, equals(AppColors.surfacePrimary));
      expect(resolvedTheme.colorScheme.onSecondary, equals(AppColors.contentPrimary));
      expect(resolvedTheme.colorScheme.onSurface, equals(AppColors.contentPrimary));

      // Typography families preservation
      expect(resolvedTheme.textTheme.displayLarge?.fontFamily, equals(AppTypography.displayFamily));
      expect(resolvedTheme.textTheme.displayMedium?.fontFamily, equals(AppTypography.displayFamily));
      expect(resolvedTheme.textTheme.displaySmall?.fontFamily, equals(AppTypography.displayFamily));
      expect(resolvedTheme.textTheme.headlineLarge?.fontFamily, equals(AppTypography.displayFamily));
      expect(resolvedTheme.textTheme.headlineMedium?.fontFamily, equals(AppTypography.displayFamily));
      expect(resolvedTheme.textTheme.headlineSmall?.fontFamily, equals(AppTypography.displayFamily));
      expect(resolvedTheme.textTheme.titleLarge?.fontFamily, equals(AppTypography.displayFamily));
      expect(resolvedTheme.textTheme.titleMedium?.fontFamily, equals(AppTypography.displayFamily));
      expect(resolvedTheme.textTheme.titleSmall?.fontFamily, equals(AppTypography.displayFamily));
      expect(resolvedTheme.textTheme.bodyLarge?.fontFamily, equals(AppTypography.mainFamily));
      expect(resolvedTheme.textTheme.bodyMedium?.fontFamily, equals(AppTypography.mainFamily));
      expect(resolvedTheme.textTheme.bodySmall?.fontFamily, equals(AppTypography.mainFamily));

      final title = tester.widget<Text>(find.byKey(const ValueKey('theme-title')));
      final body = tester.widget<Text>(find.byKey(const ValueKey('theme-body')));
      ...
      expect(lotteryNumber.style?.fontFamily, equals(AppTypography.displayFamily));

      // AppBar resolved properties
      expect(resolvedTheme.appBarTheme.backgroundColor, equals(AppColors.surfacePrimary));
      expect(resolvedTheme.appBarTheme.foregroundColor, equals(AppColors.contentPrimary));
      expect(resolvedTheme.appBarTheme.elevation, equals(0));
      expect(resolvedTheme.appBarTheme.scrolledUnderElevation, equals(0));
      expect(resolvedTheme.appBarTheme.surfaceTintColor, equals(AppColors.surfaceTintTransparent));
      ...
      expect(resolvedButtonStyle?.backgroundColor?.resolve({}), equals(AppColors.brandPrimary));
      expect(resolvedButtonStyle?.foregroundColor?.resolve({}), equals(AppColors.surfacePrimary));
      final resolvedButtonShape = resolvedButtonStyle?.shape?.resolve({}) as RoundedRectangleBorder?;
      expect(resolvedButtonShape, isNotNull);
      expect(resolvedButtonShape?.borderRadius, equals(BorderRadius.circular(8)));
      ...
      final resolvedCardShape = resolvedCardTheme.shape as RoundedRectangleBorder?;
      expect(resolvedCardShape, isNotNull);
      expect(resolvedCardShape?.borderRadius, equals(BorderRadius.circular(8)));

      // TextField resolved hint color from InputDecorationTheme
      final resolvedInputTheme = resolvedTheme.inputDecorationTheme;
      expect(resolvedInputTheme.hintStyle?.color, equals(AppColors.contentPlaceholder));
      expect(
