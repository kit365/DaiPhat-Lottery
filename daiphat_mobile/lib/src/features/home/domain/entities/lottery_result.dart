...
class LotteryResult {
  ...
  List<LotteryPrizeRow> get prizeRows => [
        ...
        LotteryPrizeRow(
          label: 'Giải bảy',
          values: prizes.seventh,
          highlight: true,
        ),
        LotteryPrizeRow(
          label: 'Giải tám',
          values: prizes.eighth,
          highlight: true,
        ),
      ];
...
List<LotoRowData> calculateLotoRows(Iterable<String> numbers) {
  ...
  for (final number in allNumbers) {
    final lastTwo =
        number.length >= 2 ? number.substring(number.length - 2) : number;
    if (lastTwo.length != 2) {
      continue;
    }
...
class LotteryResultLiveItemApiResponse {
  ...
  factory LotteryResultLiveItemApiResponse.fromJson(
    ...
    return LotteryResultLiveItemApiResponse(
      ...
      details: detailsJson
          .map(
            (item) => LotteryResultDetailApiResponse.fromJson(
