...
export const useCreateOnlineOrder = () => {
  ...
  return useMutation({
    mutationFn: (data: CreateOnlineOrderRequest) =>
      orderService.createOnlineOrder(data),
    onSuccess: (response) => {
      if (!response.success) {
        toast.error(
      ...
      queryClient.invalidateQueries({
        queryKey: [QUERY_KEYS.CLIENT_MY_ORDERS],
      });
      queryClient.invalidateQueries({
        queryKey: [QUERY_KEYS.CLIENT_MY_TICKETS],
      });
      queryClient.invalidateQueries({
        queryKey: [QUERY_KEYS.CLIENT_LOTTERY_TICKET_SEARCH],
      });
