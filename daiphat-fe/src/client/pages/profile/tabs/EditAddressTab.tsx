...
export const EditAddressTab = () => {
    ...
    useEffect(() => {
        // Mock fetch address details
        const address = DUMMY_ADDRESSES.find(a => a.id === id);
        if (address) {
            setFormData({
            ...
            // Try to geocode the existing address to set map position
            fetch(`https://rsapi.goong.io/geocode?address=${encodeURIComponent(address.addressLine)}&api_key=${GOONG_API_KEY}`)
                .then(res => res.json())
                .then(data => {
                    if (data.results && data.results.length > 0) {
        ...
        <div className="bg-white border border-[#E5E8EB] rounded-xl shadow-sm">
            <div className="flex justify-between items-center p-6 border-b border-[#E5E8EB]">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-[20px] font-bold text-[#212B36]">Cập Nhật Địa Chỉ</h3>
                </div>
                <button 
                    onClick={() => navigate('/profile/address')}
            ...
            <div className="p-6 flex flex-col gap-6">
                ...
                >
                    Lưu Thay Đổi <ArrowUpRight size={18} />
                </button>
            </div>
        </div>
    );
