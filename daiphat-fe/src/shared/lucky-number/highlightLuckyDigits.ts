import type { LuckyDigitSegment, LuckyHighlightPattern, LuckyMatchPosition, LuckyPatternType } from "./types";
...
/** Highlight lucky digit runs inside formatted strings (currency, counts, dates). */
export const highlightDisplayString = (
    display: string,
    patterns: LuckyHighlightPattern[] | undefined
): LuckyDigitSegment[] => {
    if (!display) return [];
    ...
    const flushDigitRun = () => {
        if (!digitRun) return;
        segments.push(...highlightLuckyDigits(digitRun, patterns));
        digitRun = "";
    };
