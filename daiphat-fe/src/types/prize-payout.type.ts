...
export const resolveTicketPayoutDisplay = (
    ...
    if (ticket.payoutState === 'PAYOUT_PENDING'
        ...
        || ticket.activePayoutStatus === PrizePayoutRequestStatus.APPROVED) {
        return {
            status: 'PENDING',
            label: ticket.activePayoutStatus === PrizePayoutRequestStatus.APPROVED
    ...
    if (ticket.activePayoutStatus === PrizePayoutRequestStatus.MANUAL_RESOLUTION) {
        return {
...
export const canRequestPrizePayout = (ticket: PurchasedTicket) =>
    ...
    && ticket.activePayoutStatus !== PrizePayoutRequestStatus.MANUAL_RESOLUTION;

export const getPrizePayoutIneligibilityMessage = (ticket: PurchasedTicket): string | null => {
    if (ticket.drawResultStatus !== 'WON') {
        return null;
    }
    if (ticket.activePayoutStatus === PrizePayoutRequestStatus.PENDING
        || ticket.activePayoutStatus === PrizePayoutRequestStatus.APPROVED
