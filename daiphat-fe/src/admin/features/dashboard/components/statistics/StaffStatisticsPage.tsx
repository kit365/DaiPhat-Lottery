...
import { getDetailedStaffStats } from "@/admin/features/dashboard/services/dashboardService";
...
export const StaffStatisticsPage = () => {
    ...
    const performanceOptions: any = {
        ...
        tooltip: { y: { formatter: (val: number) => `${val} lượt đặt` } }
    };
            ...
            <Box sx={gridLayout}>
                <Box sx={{ gridColumn: { xs: 'span 12', md: 'span 7' } }}>
                    <DashboardCard sx={{ p: 3, height: '100%' }}>
                        ...
                        <Chart options={performanceOptions} series={[{ name: 'Dịch vụ đã xong', data: (ticketServicePerformance || []).map((a: any) => a.count) }]} type="bar" height={360} />
                    </DashboardCard>
                </Box>
