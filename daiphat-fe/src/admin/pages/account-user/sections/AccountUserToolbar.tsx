...
import { useUserStatuses } from "../hooks/useAccountUser";

interface AccountUserToolbarProps {
...
export const AccountUserToolbar = ({
    settings,
    onSettingsChange,
    filters,
    onFilterChange,
...
}: AccountUserToolbarProps) => {
    const { data: dynamicStatuses } = useUserStatuses();

    const filterFields = useMemo(() => {
        const statusOptions =
            dynamicStatuses && dynamicStatuses.length > 0
                ? dynamicStatuses.map((s) => ({
                      value: s.value,
                      label: s.label || STATUS_LABELS[s.value] || s.value,
                  }))
