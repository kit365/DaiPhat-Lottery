...
**Local docker (`ai-ticket-ocr` container):** drop `best.pt` here; compose mounts this
directory into the `ai-ticket-ocr` service. Restart:
...
```bash
copy %USERPROFILE%\Downloads\best.pt daiphat-ai\ai-ticket-ocr\models\best.pt
```
...
`/app/models`, so retraining only needs a container restart, not a rebuild.

**Deploy** — the Dockerfile's `COPY . ./` already bakes in whatever is in this
directory (`.dockerignore` re-includes `models/*.pt`), producing an immutable
image. CD builds download the weights here from `TICKET_VISION_WEIGHTS_URL`.
