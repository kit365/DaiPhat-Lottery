...
class StationTemplateMetadata(BaseModel):
    """Effective OCR template for one active station (legacy template strategy)."""

    stationId: int
    templateId: int | None = None
    fieldLayouts: list[FieldLayoutMetadata] = Field(default_factory=list)
    # Photo the layouts were drawn on; the paper edges inside its ticketFrame
    # are the reference outline for every field box.
    sampleImageUrl: str | None = None
...
class ScanMetadata(BaseModel):
    ...
    templateId: int | None = None
    fieldLayouts: list[FieldLayoutMetadata] = Field(default_factory=list)
    # Per-station templates: legacy OCR reads the station name first, then
    # locates fields with that station's layouts.
    stationTemplates: list[StationTemplateMetadata] = Field(default_factory=list)
