...
# ai-ticket-ocr/ -- this file is domain/detection/factory.py.
...
def resolve_model_path(configured_path: str) -> str:
    ...
    TICKET_VISION_YOLO_MODEL_PATH is written relative to the service
    directory (e.g. "models/best.pt"), but the process's working directory
    is not guaranteed to be that directory (IDE runs, pytest from another
    folder) -- so a bare relative path could miss. Absolute paths are
