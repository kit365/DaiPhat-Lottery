...
class YoloLlmGuidance:
    ...
    """Oriented ticket quads (TL, TR, BR, BL) aligned with ``ticket_boxes``."""
    ticket_corners: list[list[tuple[int, int]]] = field(default_factory=list)
...
def build_yolo_llm_guidance(
    ...
    *,
    ...
    raw_quads = getattr(obb, "xyxyxyxy", None)
    quads = to_numpy(raw_quads) if raw_quads is not None else None
    ...
    quad_by_box: dict[bytes, np.ndarray] = {}

    for det_index, (box, class_id, confidence) in enumerate(zip(boxes, classes, confidences)):
        raw_name = str(names.get(int(class_id), "")).strip().lower()
        conf = float(confidence)
        if raw_name == ticket_class:
            ticket_boxes.append((conf, box))
            if quads is not None and det_index < len(quads):
                quad_by_box[np.asarray(box).tobytes()] = np.asarray(quads[det_index])
            continue
    ...
    ticket_corners: list[list[tuple[int, int]]] = []
    ...
    for index, (conf, box) in enumerate(ticket_boxes):
        ...
        ticket_corners.append(_ticket_quad(box, quad_by_box.get(np.asarray(box).tobytes())))
        hint_lines.append(
    ...
    return YoloLlmGuidance(
        ...
        ticket_corners=ticket_corners,
    )


def _ticket_quad(box: np.ndarray, quad: np.ndarray | None) -> list[tuple[int, int]]:
    """OBB quad ordered TL/TR/BR/BL, or the axis-aligned box corners."""
    if quad is not None and np.asarray(quad).size == 8:
        try:
            return order_corners(np.asarray(quad, dtype="float32"))
        except Exception:  # noqa: BLE001
            pass
    x1, y1, x2, y2 = (int(round(float(v))) for v in box)
