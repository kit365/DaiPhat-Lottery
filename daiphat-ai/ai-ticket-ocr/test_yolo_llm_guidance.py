...
def test_build_yolo_llm_guidance_returns_ordered_obb_corners(image, monkeypatch):
    monkeypatch.setattr(guidance_mod.settings, "TICKET_VISION_LLM_YOLO_GUIDANCE", True)
    monkeypatch.setattr(guidance_mod.settings, "TICKET_VISION_YOLO_TICKET_MIN_CONFIDENCE", 0.35)
    monkeypatch.setattr(guidance_mod.yolo_model, "is_available", lambda _path: True)
    obb = _FakeObb(
        boxes=[[20, 40, 260, 420], [40, 200, 240, 260]],
        cls=[3, 7],
    )
    # Slightly rotated ticket quad, starting from bottom-right (model order).
    obb.xyxyxyxy = np.asarray(
        [
            [[255, 420], [25, 410], [20, 40], [260, 50]],
            [[40, 200], [240, 200], [240, 260], [40, 260]],
        ],
        dtype="float32",
    )
    monkeypatch.setattr(
        guidance_mod.yolo_model, "load_model", lambda _path: _FakeModel(_FakeResult(obb))
    )

    result = build_yolo_llm_guidance(image, max_tickets=5, encode_crops=False)

    assert result.ticket_count == 1
    assert result.ticket_corners == [[(20, 40), (260, 50), (255, 420), (25, 410)]]


def test_build_yolo_llm_guidance_falls_back_to_aabb_corners_without_quads(image, monkeypatch):
    monkeypatch.setattr(guidance_mod.settings, "TICKET_VISION_LLM_YOLO_GUIDANCE", True)
    monkeypatch.setattr(guidance_mod.settings, "TICKET_VISION_YOLO_TICKET_MIN_CONFIDENCE", 0.35)
    monkeypatch.setattr(guidance_mod.yolo_model, "is_available", lambda _path: True)
    monkeypatch.setattr(
        ...
        "load_model",
        lambda _path: _FakeModel(
            _FakeResult(_FakeObb(boxes=[[20, 40, 260, 420], [40, 200, 240, 260]], cls=[3, 7]))
        ),
    )

    result = build_yolo_llm_guidance(image, max_tickets=5, encode_crops=False)

    assert result.ticket_corners == [[(20, 40), (260, 40), (260, 420), (20, 420)]]
