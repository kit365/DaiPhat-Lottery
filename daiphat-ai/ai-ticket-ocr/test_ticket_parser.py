...
def test_draw_date_digits_never_become_the_lottery_number(sample_stations):
    parser = _parser(sample_stations)
    # The date is printed twice, lower than the face number; one print lost
    # its separators. Its digits used to yield "230820" via a sliding window.
    ocr_by_region = {
        "whole": [
            OcrTextResult(text="4 2 4 9 4 4", confidence=0.9, y_center=0.78),
            OcrTextResult(text="23082026", confidence=0.95, y_center=0.90),
            OcrTextResult(text="23-08-2026", confidence=0.95, y_center=0.88),
        ],
    }
    result = parser.parse(ocr_by_region)
...
def test_draw_date_field_accepts_dropped_separators(sample_stations):
    parser = _parser(sample_stations)
...
def test_template_fields_ignore_whole_ticket_guesses(sample_stations):
    ...
    ocr_by_region = {
        "whole": [
            OcrTextResult(text="Cần Thơ", confidence=0.95, y_center=0.1),
            OcrTextResult(text="654321", confidence=0.99, y_center=0.9),
            OcrTextResult(text="50K1", confidence=0.95, y_center=0.8, x_center=0.2),
        ],
        "field:numbers": [OcrTextResult(text="424944", confidence=0.8)],
    }
    guessed = parser.parse(ocr_by_region)
    assert guessed.extracted.numbers == "654321"

    owned = parser.parse(ocr_by_region, template_fields=frozenset({"numbers", "batchCode", "stationName"}))
    assert owned.extracted.numbers == "424944"
    assert owned.extracted.batchCode is None
    assert owned.extracted.stationName == "Cần Thơ"
