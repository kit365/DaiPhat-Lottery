...
public abstract class AuthTestBase {
    ...
    @Mock protected UserServicePort userService;
    @Mock protected UserRepositoryPort userRepositoryPort;
    @Mock protected IdentityManagementPort identityManagementPort;
    @Mock protected AuthProperties authProperties;
    @Mock protected LoginAttemptPort loginAttemptService;
    @Mock protected RateLimiterPort rateLimiterService;
    @Mock protected DistributedLockPort lockManager;
    ...
    protected static final String DEFAULT_FRONTEND_URL = "https://fe-" + java.util.UUID.randomUUID().toString().substring(0, 8) + ".daiphat.com";
    ...
    protected void setUp() {
        ...
        lenient().when(policyConfig.getPattern()).thenReturn("^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@$!%*?&])[A-Za-z\\d@$!%*?&]{8,32}$");
 
        lenient().when(lockoutProps.getLockTimeout()).thenReturn(Duration.ofSeconds(0));
 
        // Rate Limiter defaults
        lenient().when(rateLimiterService.checkAndRecordFixed(anyString(), any(AuthAction.class), anyInt(), anyLong()))
                .thenReturn(true);
