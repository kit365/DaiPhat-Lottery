...
public class SampleOrderTransactionSeedInitializer implements ApplicationRunner {
    ...
    private static final String SEED_STATION_NAME = "Ve so sample seed";
    ...
    private void releaseAllocatedSerials(OrderDetailEntity detail) {
        List<OrderDetailSerialEntity> allocations =
                orderDetailSerialRepository.findByOrderDetailIdWithSerial(detail.getId());
        if (!allocations.isEmpty()) {
            for (OrderDetailSerialEntity allocation : allocations) {
                if (allocation.getLotteryTicketSerial() != null && allocation.getLotteryTicketSerial().getId() != null) {
                    lotteryTicketServicePort.returnSoldTicketForOrder(allocation.getLotteryTicketSerial().getId());
                }
            }
            return;
        }
    ...
    private void seedPaidOrder(
        ...
        for (PurchaseLine line : purchaseLines) {
            ...
            List<OrderDetailSerialEntity> allocations = new ArrayList<>();
            ...
            detail.setAllocatedSerials(allocations);
            details.add(detail);
        ...
        return lotteryStationRepository.findAll().stream()
                ...
                .orElseGet(() -> lotteryStationRepository.save(
                        LotteryStationEntity.builder()
                                ...
                                .inventoryCount(100)
                                .drawDays(List.of(DayOfWeek.MONDAY, DayOfWeek.WEDNESDAY, DayOfWeek.FRIDAY))
                                ...
                                .approvedBy(operator)
                                .approvedAt(seedBase)
                                .description("Station for sample order seed.")
                                .createdAt(seedBase)
                                .updatedAt(seedBase)
                                .createdBy(SEED_ACTOR)
                                .lastModifiedBy(SEED_ACTOR)
