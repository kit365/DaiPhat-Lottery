...
public class OrderSeedInitializer implements ApplicationRunner {
    ...
    private void seedAvailableTickets(UserEntity operator, LotteryStationEntity orderStation, SeedTime time) {
        ...
        for (LocalDate drawDate : List.of(time.today(), time.tomorrow())) {
            for (LotteryStationEntity station : findStationsForDrawDate(drawDate)) {
                if (station.getId().equals(orderStation.getId())) {
                    continue;
                }
                seedAvailableTicketsForDate(operator, station, drawDate, time);
            }
        }
    }
    ...
    private void seedAvailableTicketsForDate(
        ...
        String dailySeedPrefix = "SEED-AVAILABLE-"
    ...
    private void createSeedAvailableTicket(
        ...
        String numbers = String.format("%06d", (index * 111_111 + Math.floorMod(station.getId().intValue() * 17, 1000)) % 1_000_000);
    ...
    private void seedOnlinePendingPaymentOrder(
        ...
        OrderDetailEntity detail = buildDetail(order, ticketSerial, station.getPrice(), OrderDetailStatus.PROXY_HOLDING, createdAt);
    ...
    private void seedPreparingFullReplaceOrder(
        ...
        List<OrderDetailEntity> details = List.of(
                buildDetail(order, serial1, unitPrice, OrderDetailStatus.PROXY_HOLDING, paidAt),
                buildDetail(order, serial2, unitPrice, OrderDetailStatus.PROXY_HOLDING, paidAt),
                buildDetail(order, serial3, unitPrice, OrderDetailStatus.PROXY_HOLDING, paidAt)
        );
        TransactionEntity transaction = buildOnlineTransaction(
    ...
    private void seedPreparingPartialReplaceOrder(
        ...
        List<OrderDetailEntity> details = List.of(
                buildDetail(order, replaceable1, unitPrice, OrderDetailStatus.PROXY_HOLDING, paidAt),
                buildDetail(order, replaceable2, unitPrice, OrderDetailStatus.PROXY_HOLDING, paidAt),
                buildDetail(order, noReplacement, unitPrice, OrderDetailStatus.PROXY_HOLDING, paidAt)
        );
        TransactionEntity transaction = buildOnlineTransaction(
                order, totalAmount, TransactionStatus.COMPLETED,
    ...
    private void seedReplacedTicketOrder(
        ...
        OrderDetailEntity detail = buildDetail(order, oldTicketSerial, station.getPrice(), OrderDetailStatus.PROXY_HOLDING, paidAt);
