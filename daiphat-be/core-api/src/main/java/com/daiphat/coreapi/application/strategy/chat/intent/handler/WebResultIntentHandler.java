...
import com.daiphat.coreapi.application.util.chat.ChatIntentTextUtils;
...
public class WebResultIntentHandler implements ChatIntentHandler {
    ...
    private String buildReply(ChatIntentContext ctx) {
        ...
        if (region == null || region.isBlank()) {
            region = ChatIntentTextUtils.extractRegion(message);
        }
        if (region == null || region.isBlank()) {
            region = "MIEN_NAM";
        }
    ...
    private String formatSummary(LotteryResultBoardSummaryResponse summary, String ticketNumber) {
        ...
        reply.append("Kết quả xổ số ")
                .append(formatRegion(summary.region()))
                .append(" ngày ")
